        ...
        <header class="bg-emerald-600 text-white p-5 sticky top-0 z-50 shadow-md rounded-b-xl">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm opacity-90">Hai, {{ auth()->user()->name ?? 'Guest' }}</p>
                </div>
            ...
            @forelse($products as $product)
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-start">
                    <div class="flex-1">
                        ...
                        @if ($product->barcode)
                            <p class="text-xs text-gray-500 mt-1">Barcode: {{ $product->barcode }}</p>
                        @endif
                    </div>
                    ...
                    <div class="text-right flex flex-col items-end justify-between min-h-full">
                        ...
                        @if ($product->quantity <= 2)
                            <span class="bg-red-100 text-red-700 text-[10px] font-bold px-2 py-1 rounded mt-2">Stok Rendah</span>
                        @else
                            <span class="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-1 rounded mt-2">Stok Cukup</span>
                        @endif
                    </div>
                </div>
            @empty
